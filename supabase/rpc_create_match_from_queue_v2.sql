...
begin
  ...
  -- 2) gather recent opponents (cooldown: 10 minutes)
  create temporary table tmp_recent_opponents (user_id uuid primary key) on commit drop;

  insert into tmp_recent_opponents (user_id)
  select distinct opp_mtm.user_id
  from public.queue_entry_members anchor_qem
  join public.match_team_members my_mtm on my_mtm.user_id = anchor_qem.user_id
  join public.match_teams my_mt on my_mt.id = my_mtm.match_team_id
  join public.matches m on m.id = my_mt.match_id
  join public.match_teams opp_mt on opp_mt.match_id = m.id and opp_mt.side <> my_mt.side
  join public.match_team_members opp_mtm on opp_mtm.match_team_id = opp_mt.id
  where anchor_qem.queue_entry_id = p_anchor_queue_entry_id
    and m.completed_at > now() - interval '10 minutes';
  ...
  from (
    ...
    where qe.status = 'waiting' and qe.queue_type = p_queue_type and qe.id <> p_anchor_queue_entry_id
      ...
      and not exists (
        select 1 from public.queue_entry_members cand_qem
        join tmp_recent_opponents ro on ro.user_id = cand_qem.user_id
        where cand_qem.queue_entry_id = qe.id
      )
